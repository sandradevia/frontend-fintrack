...
<x-ui.modal x-data="{ open: false }" @open-profile-info-modal.window="open = true" :isOpen="false" class="max-w-[700px]">
    <div class="relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
        <div class="px-2 pr-14">
            <h4 class="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">Edit Data Pengguna</h4>
            <p class="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                Perbaiki data lembaga untuk memastikan pelaporan yang akurat.
            </p>
        </div>
        <form method="POST" action="{{ route('admin.profile.update', $dapur->id) }}" class="flex flex-col">
          @csrf
          @method('PUT')
              ...
              <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                  ...
                  @php
                      ...
                      $dapurFields = [
                          ...
                          'nomor_rekening' => 'Nomor Rekening/VA',
                          'tempat_pelaporan' => 'Tempat Pelaporan',
                      ];

                      // Fields dari tabel Periode
                      $periodeFields = [
                          'nama_periode' => 'Periode Saat Ini',
                  ...
                  {{-- Loop field Dapur --}}
                  @foreach($dapurFields as $name => $label)
                      <div>
                          ...
                          <input type="text"
                                name="{{ $name }}"
                                value="{{ old($name, $dapur->$name) }}"
                                class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                      </div>
                  @endforeach

                  {{-- Loop field Periode aktif --}}
                  @foreach($periodeFields as $name => $label)
                      <div>
                          <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                              {{ $label }}
                          </label>
                          <input type="{{ Str::contains($name, 'tanggal') ? 'date' : 'text' }}"
                                name="{{ $name }}"
                                value="{{ old($name, $dapur->periodeAktif->$name ?? '') }}"
                                class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                      </div>
                  @endforeach
    ...
    <table class="w-full table-fixed">
      ...
      <tbody class="text-sm">
        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
            <td class="w-1/3 border-b border-gray-200 px-4 py-3 font-medium text-gray-800 dark:border-gray-700 dark:text-gray-200">
                Nama Lembaga
            </td>
            <td class="border-b border-gray-200 px-4 py-3 dark:border-gray-700 dark:text-gray-100">
                {{ $dapur->nama_lembaga }}
            </td>
        </tr>

        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
            <td class="border-b border-gray-200 px-4 py-3 font-medium text-gray-800 dark:border-gray-700 dark:text-gray-200">
                Alamat
            </td>
            <td class="border-b border-gray-200 px-4 py-3 dark:border-gray-700 dark:text-gray-100">
                {{ $dapur->alamat }}
